    ...
    try {
      await prisma.product.upsert({
        ...
        update: {
          ...
          // Обновляем описание только если 1С прислала непустое значение
      ...
      if (err.code === "P2003") {
        // Категория или бренд не найдены — пробуем без них
        try {
          await prisma.product.upsert({
            where: { guid: String(product.Ид) },
            update: {
              article: product.Код ? String(product.Код) : null,
              name: String(product.Наименование || ""),
              ...(description !== null ? { description } : {}),
          ...
          console.error("Пропущен товар:", product.Ид, e2.message);
        }
      } else {
        console.error("Пропущен товар:", product.Ид, err.message);
      }
    }
  }
